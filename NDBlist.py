...
plugin         = Plugin()
addon          = xbmcaddon.Addon("plugin.video.binhbot")
pluginrootpath = "plugin://plugin.video.binhbot"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.binhbot/(.+?)/.+?\://", item["path"]):
				match = re.search("plugin.video.binhbot(/.+?/).+?\://", item["path"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.binhbot" in item["path"]:
